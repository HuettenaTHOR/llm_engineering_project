...
def grade_metrics(out_path: str) -> dict:
    """Objective-validity metrics from a run's ``.grades.jsonl`` sibling (RQ plan C2). ``Val_obj``
    (final CF, i.e. after the loop) is the PRIMARY metric; ``first_val_obj_rate`` is the same
    metric for iteration 0's candidate (the single-shot-equivalent CF, graded from the SAME run --
    see haiku_grader.grade_run). All None when no grades file exists, so the evaluator stays
    backwards-compatible with ungraded runs."""
...
def delta_val_obj_within(out_path: str) -> dict:
    ...
    paired within ONE run's grades file (RQ plan T). A max_loops=N run already contains the
    single-shot CF as iteration 0 (same per-item seed -> identical first attempt), so no separate
    single-shot CF run is needed -- see haiku_grader.grade_run."""
    grades = read_records(grades_path(out_path))
    n = len(grades)
...
def solve_accuracy(*names: str):
    ...
    present (first name that resolves to a non-empty file), else None (RQ plan C4/B2)."""
    for name in names:
...
def build_model_summary(reports: list[dict], config_path: str = DEFAULT_CONFIG) -> list[dict]:
    """Per-model P1->P2->T join (RQ plan C4/E1): solve-acc, single(=1st CF)/loop(=final CF)
    Val_obj [CI], ΔVal [McNemar p], solve/minimal/edit-dist components, verifier accept-rate &
    agreement. One CF run per model (max_loops=N) supplies BOTH conditions -- see
    ``delta_val_obj_within`` -- so no ``-single``/``-loop`` run-name pairing is needed."""
