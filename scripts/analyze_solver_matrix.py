"""Analyze the completed SOLVE-task verifier-loop matrix (results/solve_verifier_loop_*.jsonl).

Reproduces the tables in docs/SOLVER_MATRIX_FINDINGS.md: per-model solve accuracy (P1 gate),
the loop's effect (1st-attempt vs final, recover/regress), and self-verifier behaviour
(catch vs rubber-stamp on wrong first answers). Also prints the two illustrative traces.

Pure analysis, stdlib only -- touches no model. Run:  python scripts/analyze_solver_matrix.py
"""
import json
import os
