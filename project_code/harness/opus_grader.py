"""Post-hoc opus grader for counterfactual runs (RQ plan C1 + C5).
CLI: ``python -m harness.opus_grader [counterfactual_config.json]`` grades every run in the config.
"""
import difflib
import json
import re
...
# Grader must be INDEPENDENT of every solver under test; opus is itself a solver (opus4.5-loop),
...
def build_prompt(question: str, candidate: str) -> str:
    """The grading contract sent to opus. Asks it to (1) re-solve the EDITED problem and (2) judge
...
def _parse_grade(text: str) -> dict:
    """Extract the first ``{...}`` block from opus's reply and json.loads it (tolerates prose or a
...
def _grade_candidate(model: AnthropicModel, question: str, target: int, candidate: str) -> dict:
    """One opus call grading a single candidate, with a couple of retries."""
