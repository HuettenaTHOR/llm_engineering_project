...
- **Secondary — human ground-truth audit:** hand-check ~50 `Val`-passing counterfactuals
  to report what fraction are genuinely correct vs. self-consistent-but-broken. This answers
  the sharper research question and turns human grading from a bottleneck into a robustness
  result.
...
```
run_generation  (GPU/API)  ──>  results/*.jsonl  ──>  compute_metrics  ──>  plots / audit
```
...
- **One JSONL record per (item × condition)** with the full trace: original question, gold
  ...
  `{candidate x_CE, solver's solve, verifier's number, verdict}`, final `Val`, `Gen`-fail flag,
  and a `human_audit` slot (null until graded). A run's exact config + git hash + seed are
...
6. **Human ground-truth audit** of ~50 `Val`-passing items.
7. **Write-up.**
