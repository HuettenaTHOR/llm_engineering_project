...
| 7 | **~30-item manual audit** of Haiku grades; report agreement %; fill `human_audit`. |
...
### D. Validation (decision 7)

**D1 — Manual audit (~30 items).** Export a random (lightly model-stratified) 30-item sample to CSV —
`item_id, model, question, candidate, target, haiku_valid, haiku_reason, human_valid(blank)`. You fill
`human_valid`; a re-import computes Haiku↔human agreement % and writes it into each record's
`human_audit`. Small script or notebook.

### E. Reporting

**E1 — Final per-model table:** `solve-acc │ single-shot Val_obj [CI] │ loop Val_obj [CI] │ ΔVal
[McNemar p] │ solve%/minimal%/edit-dist │ verifier accept-rate & agreement │ Haiku↔human agreement`.
